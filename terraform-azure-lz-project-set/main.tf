...
module "lz_vending" {
  ...
  virtual_networks = try(each.value.network.enabled, false) ? {
    vwan_spoke = {
      ...
      address_space               = compact(concat([""], flatten([for s, _ in each.value.network.address_sizes : flatten(coalesce(azurerm_network_manager_ipam_pool_static_cidr.reservations["${each.value.name}-${s}"].address_prefixes, []))])))
