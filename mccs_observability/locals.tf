locals {
  ...
  # Resolve Cloud Team group ID from Entra ID group name lookup
  cloud_team_group_id = data.azuread_group.cloud_team.object_id
  ...
  expressroute_circuit_names = [
    for k, v in var.expressroute_circuits : v.circuit_name
  ]
