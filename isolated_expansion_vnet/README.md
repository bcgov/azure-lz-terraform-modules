...
`private_nat` is the same translation contract, implemented by this module in the routable spoke. It creates a Linux NVA (IP forwarding plus iptables MASQUERADE) in an unused spoke subnet, steers expansion traffic to that VM, and SNATs isolated sources to the NVA's spoke IP. Enterprise destinations see a spoke address that vWAN already knows. The isolated prefix still must not appear in enterprise routing.
...
without advertising the isolated CIDR, using the existing
hub firewall as the hop?
...
Changing `egress.mode` destroys the previous mode's route table, routes, NAT Gateway, or spoke NVA, and creates the new mode's equivalents. Route tables are one `for_each` resource with `create_before_destroy`, so Terraform creates the new table, updates associations, then deletes the old table. Callers that create their own subnets must set those subnets' route table from `egress_route_table_id` (not a mode-specific `route_table_ids` key). Indexing `route_table_ids["none"]` or `route_table_ids["private_nat"]` leaves the previous table with no dependents, and Azure then rejects the delete with `InUseRouteTableCannotBeDeleted`.
...
- shared enterprise firewall policy (private_nat creates a spoke NVA instead; it does not modify hub firewall policy)
...
| <a name="output_egress_route_table_id"></a> [egress\_route\_table\_id](#output\_egress\_route\_table\_id) | Route table ID for the active egress mode. Null in NAT mode. Callers that create their own subnets must use this value so a mode change reassociates those subnets before the previous route table is destroyed. |
...
| <a name="output_route_table_ids"></a> [route\_table\_ids](#output\_route\_table\_ids) | Map of the active egress mode name to its route table ID. Empty in NAT mode. Prefer egress\_route\_table\_id when associating caller-managed subnets. |
