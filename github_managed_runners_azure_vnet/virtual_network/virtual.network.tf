resource "azurerm_virtual_network" "ghrunners_vnet" {
  ...
  address_space       = azurerm_network_manager_ipam_pool_static_cidr.reservations.address_prefixes // From Azure IPAM
  dns_servers         = local.dns_servers

  subnet {
    name             = var.github_hosted_runners_subnet_name
    address_prefixes = cidrsubnet(azurerm_network_manager_ipam_pool_static_cidr.reservations.address_prefixes, local.newbits, 0) // From Azure IPAM
